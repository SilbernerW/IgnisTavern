...
export default function ApiKeyModal({
  ...
  const texts = {
    zh: {
      ...
      defaultBanner: '⚠️ 默认模式使用免费模型（MiniMax-M2.7），可能会出现角色设定偏差或叙事不稳定。强烈建议切换到自定义模式，使用更强的模型（如 GPT-5.4、Claude Opus 4.7、Gemini 3 Pro、DeepSeek-V3 等）以获得最佳体验！',
      fallbackInfo: '保底模型信息',
    ...
    en: {
      ...
      defaultBanner: '⚠️ Default mode uses a free model (MiniMax-M2.7) which may produce inaccurate character settings or unstable narratives. Strongly recommend switching to Custom mode with a stronger model (e.g. GPT-5.4, Claude Opus 4.7, Gemini 3 Pro, DeepSeek-V3) for the best experience!',
          ...
          <button
            onClick={() => {
              onSave?.({
                ...
                model: mode === 'custom' ? (model.trim() || provider.defaultModel) : 'minimax/minimax-m2.7:free',
